...
/*   Updated: 2024/06/14 13:30:19 by bmarek           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
//&& check_special_symbol(&input_string[i]) == 2)
// 	// 			i++;
// 	// 		else
...
// 	// {
// 	// 	if ((i + 1) >= shell_data->tokarr_l && 
//token_array[i].type == T_PIPE)
// 	// 		return (my_error(token_array[i].word, 
//shell_data->execution_result = 2), i);
// 	// 	else if ((i + 1) >= shell_data->tokarr_l 
//&& token_array[i].type > 2)
// 	// 		return (my_error("newline", 2), 
//shell_data->execution_result = 2);
// 	// 	else if (token_array[i].type > 2 
//&& shell_data->tokarr[i + 1].type > 1)
// 	// 		return (my_error(token_array[i + 1].word, 
//shell_data->execution_result = 2), i);
// 	// 	i++;
// 	// }
...
//shell_data->exec_result = 2), i);
// 		else if (token_array[i].type > 2 && 
//(i + 1) >= shell_data->token_count)
// 			return (my_error("newline", 2), 
//shell_data->exec_result = 2);
// 		else if (token_array[i].type > 2 && 
//shell_data->token_array[i + 1].type > 1)
// 			return (my_error(token_array[i + 1].word, 
//shell_data->exec_result = 2), i);
...
// 	shell_data->command_array = ft_malloc(sizeof(t_cmd_arr_str) * 1);
// 	shell_data->command_count = 1;
// 	allocate_command_memory((t_data_counter){.arg_count = 0, 
//.input_count = heredoc_count, .output_count = 0}, 
//shell_data->command_array);
// 	// Wypełnianie cmdarr danymi z heredo
// 	i = 0;
// 	heredoc_index = -1;
// 	while (i < end_index)
// 	{
...
// 	        set_redirection_type(&shell_data->command_array[0].
//input_tokens[heredoc_index], shell_data->token_array, &i);
// 	    }
...
// 	// 		fill_redir_type(&shell_data->cmdarr[0].inp
//[++heredoc_index], shell_data->tokarr, &i);
// 	// 	i++;
// 	// }
// 	return (heredoc_count);
// }
...
// 	alloc_cmd((t_count){.arg = 0, .in = heredoc, .out = 0},
//shell_data->cmdarr);
// 	i = 0;
// 	in = -1;
// 	while (i < token_limit)
